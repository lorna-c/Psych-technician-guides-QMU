...
expName = 'posner-auto-counter'  # from the Builder filename that created this script
expInfo = {
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\LCamus\\OneDrive - Queen Margaret University\\Lab stuff\\psychopy-doc\\posner-tutorial\\auto-counter-spreadsheets\\posner-auto-counter_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "auto_counter" ---
    ...
    target = visual.ImageStim(
        win=win,
        name='target', 
        image='target.png', mask=None, anchor='center',
        ori=0.0, pos=[0,0], size=(0.5, 0.5),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
    ...
    # --- Prepare to start Routine "auto_counter" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('auto_counter.started', globalClock.getTime())
    # Run 'Begin Routine' code from code
    # this saves the group assigned to the participant in the final data file
    thisExp.addData("group", thisgroup)
    # keep track of which components have finished
    auto_counterComponents = []
    for thisComponent in auto_counterComponents:
    ...
    # --- Ending Routine "auto_counter" ---
    for thisComponent in auto_counterComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('auto_counter.stopped', globalClock.getTime())
    # the Routine "auto_counter" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # set up handler to look after randomisation of conditions etc
    trials_2 = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('group' + thisgroup + '_blocks.xlsx'),
    ...
    for thisTrial_2 in trials_2:
        ...
        for thisTrial in trials:
            ...
            # keep track of which components have finished
            trialComponents = [cue, target, mouse]
            ...
            while continueRoutine:
                ...
                if mouse.status == NOT_STARTED and t >= 0.7-frameTolerance:
                    # keep track of start time/frame for later
